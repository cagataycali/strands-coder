...
workflows. It enables prompt delegation and parallel processing by handing off prompts to
...
2. Pass context and prompts to sub-agents
...
Model Selection:
- Use us.anthropic.claude-sonnet-4-5-20250929-v1:0 for prompts
- Use us.anthropic.claude-opus-4-20250514-v1:0 for advanced reasoning prompts
...
- Use semicolons (;) to separate package groups
- Use colons (:) to separate package from tool names
- Use commas (,) to separate tool names within a package
- Examples:
  - "strands_tools:shell,editor;strands_action:use_github"
  - "strands_tools:file_read,file_write,calculator;strands_action:system_prompt"
  - "strands_action:use_github,store_in_kb"
...
result = agent.tool.create_subagent(
    ...
    provider="bedrock",
    max_tokens=60000,
    tools="strands_tools:shell,calculator",
)
...
result = agent.tool.create_subagent(
    repository="owner/repo",
    workflow_id="agent.yml",
    prompt="Complex multi-step analysis requiring deep reasoning",
    model="us.anthropic.claude-sonnet-4-5-20250929-v1:0",
    provider="bedrock",
    max_tokens=60000,
    tools="strands_tools:retrieve,use_agent",
)
```
"""
...
def create_subagent(
    ...
    workflows. It helps with prompt delegation and parallel processing by handing off
    prompts to specialized agents running in separate GitHub Actions workflows.
    ...
    Args:
        ...
        prompt: The prompt to delegate to the sub-agent (required only for "create" action)
        ...
        model: The model ID to use for the sub-agent. Available models:
            ...
            - "us.anthropic.claude-sonnet-4-5-20250929-v1:0" (max 65536 tokens) - 10000 ideal
        ...
        tools: Tool configuration string in format "package:tool1,tool2;package2:tool3,tool4"
            Use semicolons to separate package groups, colons to separate package from tools,
            and commas to separate tool names within a package.
            Examples:
              - "strands_tools:shell,editor;strands_action:use_github"
              - "strands_tools:file_read,file_write,calculator"
    ...
    Example:
        ...
        result = create_subagent(
            ...
            provider="bedrock",
            max_tokens=10000,
            tools="strands_tools:shell,retrieve,use_agent",
        )
        ...
        result = create_subagent(
            repository="owner/repo",
            workflow_id="agent.yml",
            prompt="Analyze this dataset and provide comprehensive insights",
            model="us.anthropic.claude-sonnet-4-5-20250929-v1:0",
            provider="bedrock",
            max_tokens=10000,
            tools="strands_tools:shell,calculator,retrieve",
        )
        ...
        result = create_subagent(
            repository="owner/repo",
            workflow_id="agent.yml",
            prompt="Use custom agent logic for this prompt",
    ...
    try:
        ...
        if action == "create":
            # Validate that prompt is provided for create action
            if not prompt:
                return {
                    "status": "error",
                    "content": [
                        {
                            "text": "Error: 'prompt' parameter is required for the create action"
                        }
                    ],
                }
            ...
            # Create prompt context combining prompt and context
            prompt_context = f"prompt: {prompt}"
            if context is not None:
                prompt_context += f"\nContext: {context}"
            inputs["prompt"] = prompt_context
            ...
            if result["success"]:
                content = [
                    ...
                    {"text": f"Prompt delegated: {prompt}"},
            ...
            if result["success"]:
                ...
                if result.get("status") == "completed":
                    ...
                    if logs_result["success"]:
                        ...
                        if agent_responses:
                            content.append({"text": "Agent prompts:"})
                            for idx, response in enumerate(agent_responses, 1):
                                content.append(
                                    {
                                        "text": f"prompt {idx}: {response.get('prompt', 'N/A')}"
                                    }
                                )
